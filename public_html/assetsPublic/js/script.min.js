...
$(document).ready(function(){carousel_slider();slick_slider()});$("#submitButton").on("click",function(event){event.preventDefault();var mydata=$("form").serialize();$.ajax({type:"POST",dataType:"json",url:"contact.php",data:mydata,success:function(data){if(data.type==="error"){$("#alert-msg").removeClass("alert, alert-success");$("#alert-msg").addClass("alert, alert-danger")}else{$("#alert-msg").addClass("alert, alert-success");$("#alert-msg").removeClass("alert, alert-danger");$("#first-name").val("Enter Name");$("#email").val("Enter Email");$("#phone").val("Enter Phone Number");$("#subject").val("Enter Subject");$("#description").val("Enter Message")}
$("#alert-msg").html(data.msg);$("#alert-msg").show()},error:function(xhr,textStatus){alert(textStatus)}})});$('.content-popup').magnificPopup({type:'inline',preloader:!0,mainClass:'mfp-zoom-in',});$('.image_gallery').each(function(){$(this).magnificPopup({delegate:'a',type:'image',gallery:{enabled:!1,},})});$('.popup-ajax').magnificPopup({type:'ajax',callbacks:{ajaxContentAdded:function(){carousel_slider();slick_slider()}}});$('.video_popup, .iframe_popup').magnificPopup({type:'iframe',removalDelay:160,mainClass:'mfp-zoom-in',preloader:!1,fixedContentPos:!1});if($('select').length){$.each($('select'),function(i,val){var $el=$(val);if($el.val()===""){$el.addClass('first_null')}
if(!$el.val()){$el.addClass('not_chosen')}
$el.on('change',function(){if(!$el.val())
$el.addClass('not_chosen');else $el.removeClass('not_chosen')})})}
if($(".fit-videos").length>0){$(".fit-videos").fitVids({customSelector:"iframe[src^='https://w.soundcloud.com']"})}
if($(".custome_select").length>0){$(document).ready(function(){$(".custome_select").msDropdown()})}
$('.countdown_time').each(function(){var endTime=$(this).data('time');$(this).countdown(endTime,function(tm){$(this).html(tm.strftime('<div class="countdown_box"><div class="countdown-wrap"><span class="countdown days">%D </span><span class="cd_text">Days</span></div></div><div class="countdown_box"><div class="countdown-wrap"><span class="countdown hours">%H</span><span class="cd_text">Hours</span></div></div><div class="countdown_box"><div class="countdown-wrap"><span class="countdown minutes">%M</span><span class="cd_text">Minutes</span></div></div><div class="countdown_box"><div class="countdown-wrap"><span class="countdown seconds">%S</span><span class="cd_text">Seconds</span></div></div>'))})});$('.shorting_icon').on('click',function(){if($(this).hasClass('grid')){$('.shop_container').removeClass('list').addClass('grid');$(this).addClass('active').siblings().removeClass('active')}else if($(this).hasClass('list')){$('.shop_container').removeClass('grid').addClass('list');$(this).addClass('active').siblings().removeClass('active')}
$(".shop_container").append('<div class="loading_pr"><div class="mfp-preloader"></div></div>');setTimeout(function(){$('.loading_pr').remove();$container.isotope('layout')},800)});$(function(){$('[data-toggle="tooltip"]').tooltip({trigger:'hover',})});$(function(){$('[data-toggle="popover"]').popover()});$('.product_color_switch span').each(function(){var get_color=$(this).attr('data-color');$(this).css("background-color",get_color)});$('.product_color_switch span,.product_size_switch span').on("click",function(){$(this).siblings(this).removeClass('active').end().addClass('active')});var image=$('#product_img');var zoomActive=!1;zoomActive=!zoomActive;if(zoomActive){if($(image).length>0){$(image).elevateZoom({easing:!1,gallery:'pr_item_gallery',zoomType:"inner",galleryActiveClass:"active"})}}else{$.removeData(image,'elevateZoom');$('.zoomContainer:last-child').remove()}
$.magnificPopup.defaults.callbacks={open:function(){$('body').addClass('zoom_image')},close:function(){setTimeout(function(){$('body').removeClass('zoom_image');$('body').removeClass('zoom_gallery_image');$('.zoomContainer').slice(1).remove()},100)}};var galleryZoom=$('#pr_item_gallery');galleryZoom.magnificPopup({delegate:'a',type:'image',gallery:{enabled:!0},callbacks:{elementParse:function(item){item.src=item.el.attr('data-zoom-image')}}});$('.product_img_zoom').on('click',function(){var atual=$('#pr_item_gallery a').attr('data-zoom-image');$('body').addClass('zoom_gallery_image');$('#pr_item_gallery .item').each(function(){if(atual==$(this).find('.product_gallery_item').attr('data-zoom-image')){return galleryZoom.magnificPopup('open',$(this).index())}})});$('.plus').on('click',function(){if($(this).prev().val()){$(this).prev().val(+$(this).prev().val()+1)}});$('.minus').on('click',function(){if($(this).next().val()>1){if($(this).next().val()>1)$(this).next().val(+$(this).next().val()-1)}});$('#price_filter').each(function(){var $filter_selector=$(this);var a=$filter_selector.data("min-value");var b=$filter_selector.data("max-value");var c=$filter_selector.data("price-sign");$filter_selector.slider({range:!0,min:$filter_selector.data("min"),max:$filter_selector.data("max"),values:[a,b],slide:function(event,ui){$("#flt_price").html(c+ui.values[0]+" - "+c+ui.values[1]);$("#price_first").val(ui.values[0]);$("#price_second").val(ui.values[1])}});$("#flt_price").html(c+$filter_selector.slider("values",0)+" - "+c+$filter_selector.slider("values",1))});$(document).ready(function(){$('.star_rating span').on('click',function(){var onStar=parseFloat($(this).data('value'),10);var stars=$(this).parent().children('.star_rating span');for(var i=0;i<stars.length;i++){$(stars[i]).removeClass('selected')}
for(i=0;i<onStar;i++){$(stars[i]).addClass('selected')}})});$('.create-account,.different_address').hide();$('#createaccount:checkbox').on('change',function(){if($(this).is(":checked")){$('.create-account').slideDown()}else{$('.create-account').slideUp()}});$('#differentaddress:checkbox').on('change',function(){if($(this).is(":checked")){$('.different_address').slideDown()}else{$('.different_address').slideUp()}});$(document).ready(function(){$('[name="payment_option"]').on('change',function(){var $value=$(this).attr('value');$('.payment-text').slideUp();$('[data-method="'+$value+'"]').slideDown()})});$(window).on('load',function(){setTimeout(function(){$("#onload-popup").modal('show',{},500)},3000)});$('.heart').on('click',function(){var commerce_id=$(this).data('commerce')
var user_id=$(this).data('user')
if($(this).hasClass('like')){$(this).addClass('dislike');$(this).removeClass('like');$(this).html('<i class="fas fa-heart" style="font-size:20px;"></i>');$.ajax({headers:{'X-CSRF-TOKEN':$('meta[name="_token"]').attr('content')},url:'/api/like',method:'POST',data:{commerce_id:commerce_id,user_id:user_id},success:function(){$(this).addClass('dislike');$(this).removeClass('like');$(this).html('<i class="fas fa-heart" style="font-size:20px;"></i>')},error:function(error){console.log(error)
$(this).addClass('like');$(this).removeClass('dislike');$(this).html('<i class="far fa-heart" style="font-size:20px;"></i>')}})}else{var commerce_id=$(this).data('commerce')
var user_id=$(this).data('user')
$(this).removeClass('dislike');$(this).addClass('like');$(this).html('<i class="far fa-heart" style="font-size:20px;"></i>');$.ajax({headers:{'X-CSRF-TOKEN':$('meta[name="_token"]').attr('content')},url:'/api/dislike',method:'POST',data:{commerce_id:commerce_id,user_id:user_id},success:function(){$(this).removeClass('dislike');$(this).addClass('like');$(this).html('<i class="far fa-heart" style="font-size:20px;"></i>')},error:function(error){console.log(error)
$(this).removeClass('like');$(this).addClass('dislike');$(this).html('<i class="fas fa-heart" style="font-size:20px;"></i>')}})}});$('.star').on('click',function(){var value=$(this).data('value');$('#rating').val(value)});var colorSuccess='#28a745';var colorError='#dc3545';if($('#solicitarCodigo').length){$('#solicitarCodigo').ajaxForm({success:(data)=>{if(data.message){$('#user_id').val(data.user.id);$('.email-container').addClass('d-none');$('.code-container').removeClass('d-none');return data.message&&Swal.fire({icon:'success',title:'Enviado con éxito',text:data.message,confirmButtonText:"He Recibido Mi Código",confirmButtonColor:colorSuccess})}
return Swal.fire({icon:'error',title:'Ha ocurrido un error!',text:data.error,confirmButtonText:"Listo",confirmButtonColor:colorError})},error:(err)=>{Swal.fire({icon:'error',title:'Ha ocurrido un error!',confirmButtonText:"Listo",confirmButtonColor:colorError})
console.log(err)},})}
if($('#solicitarCodigo').length){$('#comprobarCodigo').ajaxForm({success:(data)=>{if(data.error){return Swal.fire({icon:'error',title:'Ha ocurrido un error!',text:data.error,confirmButtonText:"Listo",confirmButtonColor:colorError})}else{return location.href="/cambiar-contraseña"}},error:(err)=>{Swal.fire({icon:'error',title:'Ha ocurrido un error!',confirmButtonText:"Listo",confirmButtonColor:colorError})
